...
void loop() {
    ...
    if (robot.pixy.ccc.numBlocks) {
        ...
        if (enemy.found) {
            ...
            enemy.angle = math.countAngle(enemy.x, enemy.y - length / 3);
        ...
        if (home.found) {
            ...
            home.angle = math.countAngle(home.x, home.y - length / 3);
...
//        Serial.print(' ');
    ...
    if (abs(current_angle) > 0.2 && abs(current_angle) < 3.15 && ball.distance < 80 && !ball_catched) {
        current_angle += math.radian(60) * math.sign(current_angle);
    }
    
    if (ball.distance < 50 && abs(current_angle) < 0.25 && !ball_catched) {
        speed = 140;
        if (abs(robot.degree - target) < 15) {
            u = ((ball.angle) / PI * 180) * 2;
        }
        if (abs(current_angle) < 0.15) {
            current_angle = 0.0;
        }
    }
...
//        speed = sqrt(pow((speed * cos(current_angle) + vy), 2) + pow((speed * sin(current_angle) + vx), 2));
    ...
    if (robot.setTimer(ball_catched_timer, 100)) {
        ...
        if (robot.setTimer(ball_catched_timer, 400)) {
            robot.runDribler(1300);
            speed = 200;
            if (home.distance > 109 ) {
        ...
        if (enemy.distance < 80 && line_catched) {
                if (abs((enemy.angle / PI * 180) * 2) < 5 && robot.setTimer(kicker_timer, 3500)) {
                   robot.hitBall();
                   kicker_timer = millis();
               }
        }
//        if (robot.setTimer(ball_catched_timer, 2000)) {
//            speed = 150;
...
//            if (enemy.distance < 80 && line_catched) {
//                speed = 0;
//                u = -((enemy.angle) / PI * 180) * 2 * 0.25;
//                if (abs(u) < 8 && robot.setTimer(kicker_timer, 4500)) {
//                   robot.hitBall();
//                   kicker_timer = millis();
//               }
//            }
//        }
    ...
    if (led_counter >= 2) {
        if (!mirror_exist) {
            ...
            if (home.found || enemy.found) {
                if (delta > 0) {
                    line_angle = enemy.angle + math.radian(40) * math.sign(enemy.angle);
                } else {
                    line_angle = home.angle - math.radian(40) * math.sign(home.angle);
                }
            } else {
    ...
    if (robot.setTimer(line_timer, 100)) {
        mirror_exist = false;
    }
    ...
    if (robot.setTimer(serial_timer, 100)) {
        String mail = "";
        mail = robot.createMail("ball d", ball.distance, mail);
        mail = robot.createMail("angle", (int(current_angle * 180 / PI)) % 360, mail);
        mail = robot.createMail("ball_angle", (int(ball.angle * 180 / PI)) % 360, mail);
        mail = robot.createMail("u", u, mail);
        mail = robot.createMail("home d", home.distance, mail);
        robot.sendMail(mail);
        serial_timer = millis();
    }  
