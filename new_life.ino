...
void loop() {
    ...
    if (robot.pixy.ccc.numBlocks) {
        ...
        if (home.found) {
            home.x = robot.pixy.ccc.blocks[signature[0]].m_x; 
            home.y = robot.pixy.ccc.blocks[signature[0]].m_y;
            home.angle = -math.countAngle(home.x, home.y);
            home.square = robot.pixy.ccc.blocks[signature[0]].m_width * robot.pixy.ccc.blocks[signature[0]].m_height;
            home.distance = math.countDistance(math.distance(home.x, home.y));
        }

        if (enemy.found) {
            enemy.x = robot.pixy.ccc.blocks[signature[0]].m_x; 
            enemy.y = robot.pixy.ccc.blocks[signature[0]].m_y;
            enemy.angle = -math.countAngle(enemy.x, enemy.y);
            enemy.square = robot.pixy.ccc.blocks[signature[0]].m_width * robot.pixy.ccc.blocks[signature[0]].m_height;
    ...
    if (abs(current_angle) > 0.3 && abs(current_angle) < 3.15 && ball.distance < 160)
        current_angle += math.radian(50) * math.sign(angle);

    if (robot.buttonPressed("l"))
        target = robot.degree;

    if (robot.buttonPressed("l") && robot.buttonPressed("r") && robot.setTimer(switch_update_timer, 5000)) {
